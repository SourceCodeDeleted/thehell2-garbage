...
struct MyTool: CMFCToolBarImages
{
    static BOOL __stdcall My_MapBmpTo3dColors( HBITMAP& hBmp, BOOL bUseRGBQUAD = TRUE, COLORREF clrSrc = (COLORREF) -1, COLORREF clrDest = (COLORREF) -1 );
    BOOL My_UpdateInternalImage( int nIndex );
    static void* Get_MapBmpTo3dColors() { return MapBmpTo3dColors; }
    static void* Get_UpdateInternalImage() { auto p = &UpdateInternalImage; return *(void**) &p; }
};
...
BOOL MyTool::My_UpdateInternalImage( int nIndex )
{
...
BOOL __stdcall MyTool::My_MapBmpTo3dColors( HBITMAP& hBmp, BOOL bUseRGBQUAD/* = TRUE*/, COLORREF clrSrc/* = (COLORREF)-1*/, COLORREF clrDest/* = (COLORREF)-1*/ )
{
    if( hBmp == NULL ){
        return FALSE;
    }
...
void* adr_MapBmpTo3dColors = 0;
void* adr_UpdateInternalImage = 0;

// CTortoiseProcApp initialization
BOOL CTortoiseProcApp::InitInstance()
{
    if( !adr_MapBmpTo3dColors && !adr_UpdateInternalImage ){ // detour slow GetPixel
        adr_MapBmpTo3dColors = MyTool::Get_MapBmpTo3dColors();
        adr_UpdateInternalImage = MyTool::Get_UpdateInternalImage();
        if( adr_MapBmpTo3dColors && adr_UpdateInternalImage ){
            DWORD OldProtect;
            VirtualProtect( adr_MapBmpTo3dColors, 12, PAGE_EXECUTE_READWRITE, &OldProtect );
            *(short*) adr_MapBmpTo3dColors = 0xB848u; // mov rax,
            *(size_t*) ((size_t) adr_MapBmpTo3dColors + 2) = (size_t) MyTool::My_MapBmpTo3dColors;
            *(short*) ((size_t) adr_MapBmpTo3dColors + 10) = 0xE0FFu; // jmp rax
            VirtualProtect( adr_UpdateInternalImage, 12, PAGE_EXECUTE_READWRITE, &OldProtect );
            *(short*) adr_UpdateInternalImage = 0xB848u; // mov rax,
            auto p = &MyTool::My_UpdateInternalImage;
            *(size_t*) ((size_t) adr_UpdateInternalImage + 2) = (size_t) *(void**) &p;
            *(short*) ((size_t) adr_UpdateInternalImage + 10) = 0xE0FFu; // jmp rax
        }
    }
#ifdef _DEBUG
	...
	sCommand = parser.GetVal(L"command");
