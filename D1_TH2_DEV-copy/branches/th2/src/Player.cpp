...
void __fastcall LoadPlayerGFX( uint playerIndex, int loadingMask )
{
	...
	uint maxAppearance = (IsExeValidating || not(classId, PC_5_FIGHTER, PC_4_ROGUE)) ? 9u : 11u;
	...
	#ifdef TH1
	if( classId == PC_4_ROGUE && !thassaMORPtr || classId == PC_5_FIGHTER && ! thgladMPQPtr ){
	...
	if( (UseLookIndex ? WeaponIndex : (appearance & 0xF)) > 9 && is(classId, PC_5_FIGHTER, PC_4_ROGUE) ){
		#ifdef TH1
	...
	for( uint animMask = 1; animMask <= 0b101111111; animMask <<= 1 ){ // 0x17F
		...
		if( true /*! IsExeValidating*/ /*&& ! UseTestTrn*/ ){
			...
			if( classId == PC_4_ROGUE && (th2 || thassaMORPtr) ){
			...
			if( classId == PC_5_FIGHTER && (th2 || thgladMPQPtr) ){
	...
	do{
		for( char* armorAnim = ArmorAnim; *armorAnim; ++armorAnim ){
			for( char* weaponAnim = WeaponAnim; *weaponAnim; ++weaponAnim ){
				#ifdef TH1
				if( classIndex == PC_4_ROGUE && !thassaMORPtr || classIndex == PC_5_FIGHTER && !thgladMPQPtr ){
...
void __fastcall StartPlayerHit( int playerIndex, int damage, int needStun )
{
	...
	switch (player.ClassID ){
		case PC_0_WARRIOR:
		case PC_5_FIGHTER:soundIndex = S_798_WAR_69;	break;
		case PC_1_ARCHER:
		case PC_4_ROGUE:	soundIndex = S_689_ROG_69;	break;
	...
	int stunResist;
	// старый код
	//if( player.ClassID == PC_5_FIGHTER ){
...
void __fastcall StartPlayerKill(int playerIndex, int a2)
{
	...
	switch (player.ClassID ){
		case PC_5_FIGHTER:	soundIndex = S_801_WAR_71;	break;
		case PC_0_WARRIOR:		soundIndex = S_11_DEAD;		break;
		case PC_1_ARCHER:
...
int __fastcall PvM_Melee( int playerIndex, uint monsterIndex )
{
	...
	case PC_0_WARRIOR:	hitChance += swapToHitPenalty + 40;	break;
	case PC_5_FIGHTER:hitChance += swapToHitPenalty + 45;	break;
	case PC_3_MONK:		hitChance += swapToHitPenalty + 50;	break;
	case PC_4_ROGUE:	hitChance += swapToHitPenalty + 55;	break;
	}
	// 0070766E
	...
	case PC_0_WARRIOR:            if( RangeRND(6, 100) < player.CharLevel / 4 + 10 ) damage = 5 * damage / 2; break;
	case PC_1_ARCHER or PC_2_MAGE: if( RangeRND(6, 100) < player.CharLevel / 2      ) damage *= 2; break;
	case PC_3_MONK:               if( RangeRND(6, 100) < player.CharLevel / 2      ) damage *= 3; break;
	case PC_4_ROGUE:           if( RangeRND(6, 100) < player.CharLevel / 4 + 5  ) damage *= 4; break;
	case PC_5_FIGHTER:          if( RangeRND(6, 100) < player.CharLevel / 2 + 10 ) damage *= 2; break;
	}
	#else
...
int __fastcall PvP_Melee( uint attackerPlayerIndex, int targetPlayerIndex )
{
	...
	if( attacker.ClassID == PC_5_FIGHTER ){
		hitChance += 35;
	}
	...
	if( attackerClass == PC_0_WARRIOR || attackerClass == PC_5_FIGHTER ){
...
bool __fastcall PM_DoAttack( uint playerIndex )
{
	...
	if( isMonkWithStaff	|| isMonkWithAxe || player.ClassID == PC_5_FIGHTER && (isAxeInHand || isTwoWeaponInHand || isTwoHandedWeapon) ){// Условие сплэш атаки
...
void __fastcall ModifyPlayerVitality(int playerIndex, int vitalityDelta)
{
	...
	if( CannotIncreaseStat() != 1 ){
		...
		case PC_1_ARCHER: lifeDelta *= 2; break;
		case PC_2_MAGE: break;
		case PC_3_MONK: lifeDelta =(lifeDelta >> 1) + 2 * lifeDelta; break;
		case PC_4_ROGUE: lifeDelta *= 2; break;
		case PC_5_FIGHTER: lifeDelta = (2 * lifeDelta >> 2) + 3 * lifeDelta; break;
		}
