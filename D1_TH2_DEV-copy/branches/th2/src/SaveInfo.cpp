...
void __fastcall SavePlayerInfo(PlayerInfo* playerInfo, int playerIndex)
{
	...
	memset( playerInfo, 0, sizeof(PlayerInfo4) );
	...
	((PlayerInfo4*)playerInfo)->subclassID = player->subclassID; //qndel - subclasses
	((PlayerInfo4*)playerInfo)->specializationID = player->specializationID; //qndel - subclasses
	...
	itemInfo = ((PlayerInfo4*)playerInfo)->newbodySlots; item = &player->OnBodySlots[7];
	for( int i = 0; i < 3; ++i ) SaveItem( itemInfo++, item++ );
	itemInfo = ((PlayerInfo4*)playerInfo)->newInvSlots; item = &player->InventorySlots[40];
	for( int i = 0; i < 30; ++i ) SaveItem( itemInfo++, item++ );
	for( int i = 0; i < 30; ++i ) ((PlayerInfo4*)playerInfo)->newInvUsed[ i ] = player->InvUsed[ i + 40 ];
	#endif // TH1
	...
	for (int i = 0; i < 1000; ++i) {
		((PlayerInfo4*)playerInfo)->someNewBigDataArray[i] = player->someNewBigDataArray[i];
	}
}
...
void __fastcall LoadCharacter(PlayerInfo* playerInfo, int playerIndex, int isNetPlayer)
{
	...
	player->subclassID = ((PlayerInfo4*)playerInfo)->subclassID;//qndel - subclasses
	player->specializationID = ((PlayerInfo4*)playerInfo)->specializationID;//qndel - subclasses
	...
	if( playerInfo->signature == 'HELL' && playerInfo->version > 0 ){
		...
		if( playerInfo->version >= 12 ){
			...
			for( int i = 0; i < 30; ++i ) player->InvUsed[ i + 40 ] = ((PlayerInfo4*) playerInfo)->newInvUsed[ i ];
			for (int i = 0; i < 1000; ++i) {
				player->someNewBigDataArray[i] = ((PlayerInfo4*)playerInfo)->someNewBigDataArray[i];
			}
		}else{
			for( int i = 40; i < 70; ++i ){
