...
int __fastcall CalculateManaNeedToCastSpell(int casterIndex, int spellIndex)
{
	...
	}else if( playerClass == PC_1_SCOUT || playerClass == PC_3_MONK || playerClass == PC_4_ASSASSIN ){
...
void __fastcall HealOther( int casterPlayerIndex, int healedPlayerIndex )
{
	...
	switch (caster.ClassID ){
		case PC_5_GLADIATOR:
		case PC_0_PALADIN:		healValue *= 2;			break;
		case PC_4_ASSASSIN:
		case PC_1_SCOUT:		healValue += healValue >> 1;	break;
