...
__forceinline void UseHealingPotion(int playerIndex)
{
	...
	switch (player.ClassID ){
		case PC_5_GLADIATOR:
		case PC_0_PALADIN: lifeAdd *= 2;	break;
		case PC_1_SCOUT:
		case PC_4_ASSASSIN:
...
void LoadInvPanel()
{
	...
	case PC_1_SCOUT:
	case PC_4_ASSASSIN:	fileName = !th2 ? "Data\\Inv\\Inv_rog.CEL"  : "Data\\Inv\\inv2x1.cel"; break; 
	case PC_0_PALADIN:
	case PC_5_GLADIATOR:fileName = !th2 ? "Data\\Inv\\Inv.CEL"      : "Data\\Inv\\inv1x1.cel"; break; 
	default:   
		return;
	}
...
bool __fastcall PutWeaponFromCursorToArm( int playerIndex )
{
	...
	if( isOneHandedWeapon ){
		...
		if( playerClass >= PC_4_ASSASSIN || isLeftHandHasNoWeapon && isRightHandHasNoWeapon ){
...
void __fastcall PutCursorItem( int playerIndex, int mX, int mY )
{
	...
	if( targetEquippedLocation == EL_1_ONE_HANDED && cursorItem.EquippedLocation == EL_2_TWO_HANDED ){
		...
		#ifdef ALLOW_TWO_HANDED_IN_ONE
		if( player.ClassID == PC_5_GLADIATOR && (cursorItem.ItemCode == IC_1_SWORD || cursorItem.ItemCode == IC_4_MACE) ){
	...
	if( targetEquippedLocation != EL_7_INVENTORY && targetEquippedLocation != EL_8_BELT && !cursorItem.IsReqMet ){
		...
		switch( player.ClassID ){
			case PC_0_PALADIN:
			case PC_5_GLADIATOR:	soundIndex = S_736_WAR_13;	break;
			case PC_1_SCOUT:
			case PC_4_ASSASSIN:		soundIndex = S_633_ROG_13;		break;
		...
		if( baseSlot == IS_5_RightHand ){
			if( bodyItem.ItemCode == IC_M1_NONE ){
				...
				if( leftHand.EquippedLocation == EL_2_TWO_HANDED
				 #ifdef ALLOW_TWO_HANDED_IN_ONE
				 && (player.ClassID != PC_5_GLADIATOR || leftHand.ItemCode != IC_1_SWORD && leftHand.ItemCode != IC_4_MACE)
				 #endif
					){
				...
				if( leftHand.ItemCode == IC_M1_NONE	|| leftHand.TypeID != cursorItem.TypeID
				 || player.ClassID >= PC_4_ASSASSIN && leftHand.TypeID == ITEM_1_WEAPON && cursorItem.TypeID == ITEM_1_WEAPON ){
					SendCmdEquipItem(false, baseSlot);
					memcpy(&bodyItem, &cursorItem, sizeof(Item));
					break;
				}
			...
			if( bodyItem.ItemCode == IC_M1_NONE ){
				if( rightHand.ItemCode == IC_M1_NONE || rightHand.TypeID != cursorItem.TypeID
				 || player.ClassID >= PC_4_ASSASSIN && rightHand.TypeID == ITEM_1_WEAPON && cursorItem.TypeID == ITEM_1_WEAPON ){
					SendCmdEquipItem(false, baseSlot);
					memcpy(&bodyItem, &cursorItem, sizeof(Item));
					break;
				}else{
			...
			if( rightHand.ItemCode != IC_M1_NONE ){
				if( rightHand.TypeID == cursorItem.TypeID && ( player.ClassID < PC_4_ASSASSIN || rightHand.TypeID != ITEM_1_WEAPON || cursorItem.TypeID != ITEM_1_WEAPON) ){
					SendCmdEquipItem(false, IS_5_RightHand);
					cursorGraphics = SwapItemsAndGetFirstItemGraphicsValue(&rightHand, &cursorItem);
					break;
				}
...
void __fastcall StartScriptWithGetupQuestItem( int playerIndex )
{
	...
	if( cursorItem.baseItemIndex == BI_17_HUGE_AXE ){
		...
		if( hugeAxeQuest.status == QS_2_IN_PROGRESS 
			&& hugeAxeQuest.status2 == QS_3_COMPLETE){
				...
				switch (player.ClassID){
				case PC_0_PALADIN:
				case PC_5_GLADIATOR:SoundMessage = S_825_WAR_SL;	break;
				case PC_2_MAGE:		SoundMessage = S_613_SOR_SL;	break;
				case PC_3_MONK:		SoundMessage = S_933_MONK_SL;		break;
				case PC_1_SCOUT:
				case PC_4_ASSASSIN:	SoundMessage = S_716_ROG_SL;		break;
				}
	...
	if( cursorItem.baseItemIndex == BI_16_ANVIL_OF_FURY ){
		...
		if( anvilOfFuryQuest.talkToFlag == 1 ){
			...
			case PC_0_PALADIN:
			case PC_5_GLADIATOR:SoundMessage = S_819_WAR_89;	break;
			case PC_2_MAGE:		SoundMessage = S_607_SOR_89;	break;
			case PC_3_MONK:		SoundMessage = S_927_MONK_89;		break;
			case PC_1_SCOUT:
			case PC_4_ASSASSIN:	SoundMessage = S_710_ROG_89;		break;
			}
		}
	}
	...
	if( cursorItem.baseItemIndex == BI_328_GOLDEN_ELIXIR ){
		...
		switch (curPlayer.ClassID){
		case PC_0_PALADIN:
		case PC_5_GLADIATOR:SoundMessage = S_1074;	break;
		case PC_2_MAGE:		SoundMessage = S_1076;	break;
		case PC_3_MONK:		SoundMessage = S_1077;		break;
		case PC_1_SCOUT:
		case PC_4_ASSASSIN:	SoundMessage = S_1075;		break;
		}
	}
	...
	if( player.ItemOnCursor.baseItemIndex == BI_9_MAGIC_ROCK ){
		...
		if( magicRockQuest.talkToFlag == 1 ){
			...
			switch (curPlayer.ClassID){
			case PC_0_PALADIN:
			case PC_5_GLADIATOR:SoundMessage = S_817_WAR_87;	break;
			case PC_2_MAGE:		SoundMessage = S_605_SOR_87;	break;
			case PC_3_MONK:		SoundMessage = S_925_MONK_87;		break;
			case PC_1_SCOUT:
			case PC_4_ASSASSIN:	SoundMessage = S_708_ROG_87;		break;
			}
		}
	}
	...
	if( (player.ItemOnCursor.UniqueItemIndex >= 178 && player.ItemOnCursor.UniqueItemIndex <= 180) 
		 || ( player.ItemOnCursor.UniqueItemIndex >= 368 && player.ItemOnCursor.UniqueItemIndex <= 376 ) ){
			...
			switch (curPlayer.ClassID){
			case PC_0_PALADIN:
			case PC_5_GLADIATOR:SoundMessage = S_821_WAR_SL;	break;
			case PC_2_MAGE:		SoundMessage = S_609_SOR_SL;	break;
			case PC_3_MONK:		SoundMessage = S_929_MONK_SL;		break;
			case PC_1_SCOUT:
			case PC_4_ASSASSIN:	SoundMessage = S_712_ROG_SL;		break;
			}
	}
	...
	if( player.ItemOnCursor.baseItemIndex == BI_22_CRYPT_MAP ){
		...
		switch (curPlayer.ClassID){
		case PC_0_PALADIN:
		case PC_5_GLADIATOR:SoundMessage = S_809_WAR_46;	break;
		case PC_2_MAGE:		SoundMessage = S_597_SOR_46;	break;
		case PC_3_MONK:		SoundMessage = S_917_MONK_46;		break;
		case PC_1_SCOUT:
		case PC_4_ASSASSIN:	SoundMessage = S_700_ROG_46;		break;
		}
	}
	...
	if( baseItem == BI_335_TORN_NOTE_1 || baseItem == BI_336_TORN_NOTE_2 || baseItem == BI_337_TORN_NOTE_3 ){
		...
		if( notesFlags == (BIT(1)|BIT(2)|BIT(3)) ){
			...
			switch (curPlayer.ClassID){
			case PC_0_PALADIN:
			case PC_5_GLADIATOR:SoundMessage = S_775_WAR_46;	break;
			case PC_2_MAGE:		SoundMessage = S_563_SOR_46;	break;
			case PC_3_MONK:		SoundMessage = S_883_MONK_46;		break;
			case PC_1_SCOUT:
			case PC_4_ASSASSIN:	SoundMessage = S_666_ROG_46;		break;
			}
	...
	if( baseItem == BI_399_SHADOWFANG ){ // fleshdoom patch (8) did
		...
		if( Quests[Q_21_FLESHDOOM].status2 >= 7 ){
			switch( curPlayer.ClassID ){
			case PC_0_PALADIN:
			case PC_5_GLADIATOR:SoundMessage = S_823_WAR_93;	break;
			...
			case PC_1_SCOUT:
			case PC_4_ASSASSIN:	SoundMessage = S_714_ROG_93;	break;
			}
		}else{
			switch( curPlayer.ClassID ){
			case PC_0_PALADIN:
			case PC_5_GLADIATOR:SoundMessage = S_816_WAR_86;	break;
			...
			case PC_1_SCOUT:
			case PC_4_ASSASSIN:	SoundMessage = S_707_ROG_86;	break;
			}
		}
	}
}
...
void __fastcall GetupItemFromMap( int playerIndex, int itemIndex )
{
	...
	if( cursorItem.ItemCode == IC_11_GOLD ){
		...
		if( (apearance == 0 || apearance == 1 || player.ClassID >= PC_4_ASSASSIN && (apearance == 6 || apearance == 2))
	...
	if( playerIndex == CurrentPlayerIndex ){
		...
		switch (player.ClassID){
		case PC_0_PALADIN:
		case PC_5_GLADIATOR:soundIndex = S_737_WAR_14;	break;
		case PC_2_MAGE:		soundIndex = S_531_SOR_14;	break;
		case PC_3_MONK:		soundIndex = S_851_MONK_14;	break;
		case PC_1_SCOUT:
		case PC_4_ASSASSIN:	soundIndex = S_634_ROG_14;	break;
		}
		if( soundIndex != S_M1_NO_SOUND ){
...
char GetSlotMouseAbove()
{
	...
	if( cell.baseSlot < IS_10_7_Inventory ){ // on body
		...
		if( cell.baseSlot == IS_5_RightHand && !(player.OnBodySlots[IS_4_LeftHand].ItemCode == IC_M1_NONE || player.OnBodySlots[IS_4_LeftHand].EquippedLocation != EL_2_TWO_HANDED
		 #ifdef ALLOW_TWO_HANDED_IN_ONE
		 || player.ClassID == PC_5_GLADIATOR && (player.OnBodySlots[IS_4_LeftHand].ItemCode == IC_1_SWORD || player.OnBodySlots[IS_4_LeftHand].ItemCode == IC_4_MACE) 
		 #endif
		 )){
...
bool __fastcall RightClickOnItem(int playerIndex, int invCellIndex)
{
	...
	if( baseItemIndex == BI_17_HUGE_AXE ){
		...
		switch (player.ClassID ){
		case PC_4_ASSASSIN:
		case PC_1_SCOUT:		SoundMessage = S_716_ROG_SL;				return true;
		case PC_5_GLADIATOR:
		case PC_0_PALADIN:		SoundMessage = S_825_WAR_SL;				return true;
	...
	if( baseItemIndex == BI_19_FUNGAL_TOME ){
		...
		switch (player.ClassID ){
		case PC_4_ASSASSIN:
		case PC_1_SCOUT:		SoundMessage = S_649_ROG_29;				return true;
		case PC_5_GLADIATOR:
		case PC_0_PALADIN:		SoundMessage = S_758_WAR_29;				return true;
	...
	switch (player.ClassID ){
	case PC_4_ASSASSIN:
	case PC_1_SCOUT:		soundIndex = S_633_ROG_13;				break;
	case PC_5_GLADIATOR:
	case PC_0_PALADIN:		soundIndex = S_736_WAR_13;				break;
	case PC_2_MAGE:			soundIndex = S_530_SOR_13;				break;
	case PC_3_MONK:			soundIndex = S_850_MONK_13;				break;
	}
	PlayGlobalSound(soundIndex);
...
void __fastcall CalcCharParams(int playerIndex, int canLoadAnimFlag)
{
	...
	int& classAttrib = (classID == PC_0_PALADIN || classID == PC_5_GLADIATOR) ? addsStrength : (classID == PC_2_MAGE ? addsMagic : addsDexterity );
	if( IsQuestFloor ){
...
LABEL_73:
	...
	int addHP;
	switch( classID ){
	case PC_5_GLADIATOR:addHP = addsVitality + 2 * addsVitality; break;
	case PC_0_PALADIN:
	case PC_4_ASSASSIN:
	case PC_3_MONK:		addHP = 2 * addsVitality + (2 * addsVitality >> 2); break;
	case PC_1_SCOUT:	addHP = addsVitality + (addsVitality >> 1); break;
	...
	case PC_4_ASSASSIN: addMana = 2 * addMana + (addMana >> 1); break;
	}
	...
	if( !IsExeValidating && is( classID, PC_4_ASSASSIN, PC_5_GLADIATOR ) && is(appearance, 2, 6) // 2 - sword, 6 - mace
	...
	if( player.OnBodySlots[IS_6_Torso].ItemCode == IC_9_HEAVY_ARMOR && player.OnBodySlots[IS_6_Torso].IsReqMet ){
		...
		case PC_4_ASSASSIN	: player.ACFromClass += player.CharLevel >> 1;	break;
		case PC_5_GLADIATOR	: player.ACFromClass += player.CharLevel >> 2;	break;
		}
    ...
    if( th2 && bd_2h ){
        if (classID == PC_5_GLADIATOR && (bd_sword || bd_mace)) {
        ...
        else if (classID == PC_0_PALADIN && (bd_sword || bd_mace)) { player.BaseDamage = (player.CharLevel*(player.CurStrength + player.CurVitality) / 45) + 5; }
    }
}
